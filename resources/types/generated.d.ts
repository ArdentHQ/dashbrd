...
paginated: {data:Array<App.Data.Articles.ArticleData>;links:Array<{url:string | null;label:string;active:boolean;}>;meta:{current_page:number;first_page_url:string;from:number | null;last_page:number;last_page_url:string;next_page_url:string | null;path:string;per_page:number;prev_page_url:string | null;to:number | null;total:number;};};
};
...
export type WalletData = {address:string;domain:string | null;avatar:WalletAvatarData;totalUsd:number;totalBalanceInCurrency:string;totalTokens:number;collectionCount:number;galleryCount:number;timestamps:{tokens_fetched_at: number|null, native_balances_fetched_at: number|null};isRefreshingCollections:boolean;canRefreshCollections:boolean;};
}
