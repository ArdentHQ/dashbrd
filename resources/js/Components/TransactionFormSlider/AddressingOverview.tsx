...
export const AddressingOverview = ({
    ...
    return (
        <div
            className="relative rounded-xl border border-theme-secondary-300"
            data-testid="AddressingOverview__Container"
        >
            <div className="border-b border-theme-secondary-300 px-6 py-4">
                <Label>{t("pages.send_receive_panel.send.from")}</Label>
                {isTruthy(fromAddress) && <Wallet address={fromAddress} />}
            </div>
            <div className="z-1 absolute right-6 top-1/2 flex h-10 w-10 -translate-y-2/4 items-center justify-center rounded-full border border-theme-secondary-300 bg-white">
                <Icon
                    name="FatArrowDown"
