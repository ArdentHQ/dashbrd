...
return [
    ...
    'before_send' => function (Event $event, ?EventHint $hint): ?Event {
        // Ignore Too Many Requests exceptions
        if ($hint !== null && $hint->exception->getCode() === 429) {
            return null;
        }

        // Ignore `MaxAttemptsExceededException`
        if ($hint !== null && $hint->exception instanceof MaxAttemptsExceededException) {
            return null;
        }
